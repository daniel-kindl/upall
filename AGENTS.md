...
Imperative mood, lowercase, no trailing period. Describe what the change does, not what
you did.
...
Versions are [semver 2.0.0](https://semver.org/). Release channels and how tags map to
branches are in [ADR-0006](adr/0006-branching-model-and-release-channels.md).

### What is public

These are the interfaces under semver. Changing one incompatibly is a breaking change:

- the CLI surface, meaning command names, flag names, and flag semantics;
- exit codes, the contract in ARCHITECTURE.md;
- the config schema, meaning TOML keys and their meanings;
- the JSON output schema, for `--json` on `plan` and `apply`;
- the provider manifest schema, meaning fields and parser names.

### What is not

**The Go API is not public.** Everything is `internal/`, so nothing outside this module
can import it, and no rename inside it is a breaking change. This is deliberate. See
...
explicitly, and the commit still needs its `!` or `BREAKING CHANGE:` footer. The marker
is how release notes are generated, so it is not optional just because the bump is
smaller.
...
`main` holds stable, tested, released code. Nothing else, ever.

`dev` is the integration branch, and itself a released channel.

`feature/*` is where work happens.
...
2. **Feature branches PR into `dev`. Never into `main`.** A pull request from
   `feature/*` targeting `main` is closed rather than retargeted, because retargeting
   is how untested work reaches `main`.
...
6. **CI green on `windows-latest` and `ubuntu-latest`** before any merge. A change that
   compiles on only one platform is not mergeable.

### Hotfixes

`hotfix/*` may PR directly to `main`. **The fix must then be back-merged to `dev` as
part of the same piece of work**, not as a follow-up, because a forgotten back-merge
silently reverts the fix at the next release.
...
`revive`'s `exported` rule fails the build on a missing comment. It cannot tell whether
the comment says anything useful, so `// Apply applies.` passes and is worthless. Write
the comment for someone who has never seen the package.

---

## Pull requests

**One milestone criterion or less per PR.** If the PR description needs the word "and",
...
roadmap is only useful if it is accurate, and a cold session trusts it completely. An
unticked box for finished work sends the next session to redo it.
