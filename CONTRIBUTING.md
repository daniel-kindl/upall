...
> **The project is at milestone M0** — design documents, no application code yet. The
> most useful contribution right now is a second opinion on
...
1. **[docs/ROADMAP.md](docs/ROADMAP.md)** — the first milestone with unchecked boxes is
   the current work. Milestones are sequential.
2. **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)** — how it fits together, and the
   rules that hold everywhere.
3. **[AGENTS.md](AGENTS.md)** — commit format, branching, PR scope. All the mechanics.

And **[adr/](adr/README.md)** before proposing anything that reverses a settled
decision. Every one of those records what it cost and what was rejected. Disagreeing
is fine — that is what a superseding ADR is for — but do it knowing what the argument
already was.
...
**Linux, for the GUI only.** `upall-gui` needs X11 and OpenGL development headers to
*build*. Nothing extra is needed to *run* it — that is the whole point of
...
rather than retargeted; `main` only ever contains released code. The full model is in
[AGENTS.md](AGENTS.md#branching).
...
Refactors ship separately from behavior changes — a diff that both moves code and
...
only when it needs an API, a non-textual protocol, or logic no parser can express —
and say why in the package's `doc.go`.

Every provider needs fixture tests built from **real captured output**, including the
nothing-to-update case and at least one error case. Run the tool by hand, paste what
it printed into a fixture file, and test the parser against that. Invented output
tests the parser against your imagination.
...
**No test may invoke a real package manager.** Fake `internal/exec` instead — a test
suite that mutates the machine running it is not a test suite. There is a fake runner
for this; use it.
