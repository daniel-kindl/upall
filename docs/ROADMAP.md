...
Milestones are sequential. Do not start one until every box above it is ticked —
...
- [x] `docs/ARCHITECTURE.md` covers the provider model, the plan/apply pipeline, the
      ...
      security model.
- [x] `docs/ROADMAP.md` (this file) has M0–M13, each with checkbox criteria, plus a
      Post-1.0 section pairing every deferred item with its revisit trigger.
- [x] `adr/README.md` explains the process; `adr/0000-template.md` is the template;
      ADRs 0001–0007 record every decision settled before code, each naming the
...
- [x] `.github/dependabot.yml`, `.github/workflows/ci.yml`, and
      `.github/workflows/codeql.yml` exist with `paths:` filters, so a doc-only
      commit triggers no Go job and leaves no red check.
...
- [ ] `.golangci.yml` enables `revive` with the `exported` rule, so a missing doc
      comment on an exported identifier fails the build.
- [ ] `go vet ./...`, `golangci-lint run`, and `go test ./...` all pass.
- [ ] CI runs build + vet + lint + test on `windows-latest` and `ubuntu-latest` for
...
- [ ] The package compiles with no reference to `os/exec`, `os.Stdout`, or `fmt.Print*`
      — the types cannot perform I/O.
- [ ] `Provider` declares `ID`, `Platforms`, `NeedsElevation`, `Detect`, `Plan`,
...
- [ ] `internal/exec` exposes a runner interface plus a real implementation; all
...
- [ ] Every call takes a `context.Context`; cancelling it kills the process, and a
      test proves the process is gone.
- [ ] Per-command timeouts are supported and surface as the `timeout` error kind.
- [ ] stdout and stderr are captured separately and returned; neither is written to
      the terminal by this package.
- [ ] A fake runner ships for tests: canned output, canned exit codes, recorded
      invocations.
- [ ] Structured logging records argv, duration, and exit code at debug level, with
      no secrets or full environment.
...
**Goal:** Both kinds of provider exist, are indistinguishable to callers, and one
works end to end on each OS.

- [ ] `internal/provider` holds a registry that resolves providers by ID and filters
      by platform.
- [ ] Native providers implement `core.Provider` directly.
- [ ] TOML manifests are loaded into an adapter satisfying the same interface; a test
      asserts the registry cannot distinguish the two.
- [ ] Manifests are embedded with `go:embed`; the binary needs no files on disk.
...
- [ ] `internal/pipeline` runs discover → detect → plan → aggregate.
- [ ] Detect and plan run concurrently across providers; a test with deliberately slow
      fakes proves total time is bounded by the slowest, not the sum.
- [ ] One provider failing to plan does not prevent others from planning.
- [ ] The pipeline emits typed progress events on a channel; no package below
...
- [ ] No provider `Apply` is reachable from the plan path; a fake that fails on
      `Apply` proves it is never called.
...
- [ ] Providers declare `NeedsElevation`; manifests express it as a field.
...
- [ ] Windows Update reports pending reboots; the run summary surfaces that a reboot
      is required and never reboots on its own.
- [ ] A machine with none of a platform's providers installed produces a clean run
      that exits 0, not an error.
...
- [ ] Docker and Podman are both detected; each is absent-tolerant independently.
- [ ] Plan lists images with a newer digest available for the tag currently in use.
- [ ] Apply pulls those images.
- [ ] Compose projects are discovered, their images updated, and only affected
      services recreated.
...
- [ ] A stopped container or a compose project that fails to come back up is reported
      as a failure with its logs, not silently left down.
...
- [ ] Providers can be enabled or disabled in config; the concurrency bound and
...
- [ ] A missing config file is normal and produces documented defaults. A malformed
      one exits 2 naming the file, line, and problem.
...
- [ ] The window stays responsive throughout; no pipeline work runs on the UI thread.
...
- [ ] Confirmation is a dialog satisfying the same confirmer interface the CLI's
      prompt satisfies.
- [ ] Elevation prompts surface as dialogs; refusing leaves the app usable and the
...
- [ ] Tagging on `main` publishes a stable release; merging to `dev` publishes a
...
- [ ] `--help` is a real deliverable: every command and flag description reads as a
      sentence, `upall --help` shows a worked example, and `upall <cmd> --help`
      states the exit codes that command can return.
- [ ] The README gets a new user from "never heard of this" to a successful run.
- [ ] A clean Windows VM and a clean Linux VM each install from the published
      artifacts and complete a real run.
...
triggers, not wishes — the point is that a future session can tell whether the moment
has arrived instead of guessing.
...
| **macOS support** | Windows and Linux are stable at v1.0. Homebrew, MAS, and softwareupdate are the providers; the manifest model should already cover the first two. |
...
| **Toolchain providers** (npm, pipx, cargo, rustup, gem) | The OS package managers are complete and stable. These are easy manifests; they are deferred for scope, not difficulty. |
| **Scheduling / daemon mode** | No trigger. Out of scope — systemd timers and Task Scheduler already do this. |
| **Remote / multi-host** | No trigger. Out of scope — this is Ansible's job. |
