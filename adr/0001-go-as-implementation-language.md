...
a table, collect results, print them. There is very little algorithmic work and
almost no performance-sensitive code — the wall clock is dominated by `apt` talking
to a mirror, not by anything upall computes.

The project is worked on in bursts with long gaps between sessions, by contributors
who arrive without context. Time-to-productive-after-six-months is a real constraint,
...
- Subprocess handling, context cancellation, and structured concurrency — the three
  things this codebase does most — are standard library, well documented, and boring.
- Coming back cold is cheap. Go's small surface and explicitness mean six-month-old
  code reads the way it was written.
- Binaries are large: 10–20 MB for the CLI, 25–40 MB for the GUI. For a tool
  installed once and run occasionally, this does not matter, but it will look
  startling next to a shell script.
- Error handling is verbose, and there is no type-system help for making illegal
  states unrepresentable. The domain — a plan that is either empty, partial, or
...
- **cgo is a liability to watch.** Pure-Go dependencies keep cross-compilation
  trivial; anything requiring cgo makes it painful. This constrains later choices,
  and did: see [ADR-0005](0005-fyne-for-the-gui-client.md).
...
Rejected on cross-compilation friction and iteration speed. Producing Windows
binaries from Linux, or vice versa, needs a linker and toolchain setup that Go gets
right by default — and this is a project where release engineering has to be
maintainable by one person returning to it after months away. Compile times also tax
the edit-run loop that dominates provider parser work.
...
Native AOT can produce self-contained binaries, and the Windows integration story —
particularly the Windows Update COM API, which is a native provider either way — is
better than anything else here. Rejected because the Linux experience remains
second-class in practice and the AOT toolchain adds a step that has to keep working
across a project with long dormant periods.
